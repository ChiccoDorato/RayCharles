...
{
    pure nothrow bool xyzIsClose(T)(in T v) const
...
{
    pure nothrow R opBinary(string op)(in T rhs) const if (op == "+" || op == "-")
...
{
    pure nothrow R opUnary(string op)() const if (op == "-")
...
{
    pure nothrow R opBinary(string op)(in float alfa) const if (op == "*")
...
{
    pure nothrow R opBinaryRight(string op)(in float alfa) const if (op == "*")
...
{
    pure nothrow float opBinary(string op)(in T rhs) const if (op == "*")
...
{
    pure nothrow R opBinary(string op)(in T rhs) constif (op == "^")
...
{
    pure nothrow float squaredNorm()() const
...
{
    pure nothrow R normalize()() const
...
struct Vec
{
    ...
    pure nothrow Vec opBinary(string op)(in Vec rhs) const if (op == "+" || op == "-")
    {
    ...
    pure nothrow Vec opBinary(string op)(in float alfa) const if (op == "*")
    {
    ...
    pure nothrow float opBinary(string op)(in Vec rhs) const if (op == "*")
    {
        return x * rhs.x + y * rhs.y + z * rhs.z;
    }
    ...
    pure nothrow float opBinary(string op)(in Normal rhs) const if (op == "*")
    {
        return x * rhs.x + y * rhs.y + z * rhs.z;
    }
    ...
    pure nothrow Vec opBinary(string op)(in Normal rhs) const if (op == "^")
    {
        return Vec(y * rhs.z - z * rhs.y, z * rhs.x - x * rhs.z, x * rhs.y - y * rhs.x);
    }
...
struct Point
{
    ...
    pure nothrow Point opBinary(string op)(in Vec rhs) const if (op == "+" || op == "-")
    {
        return mixin("Point(x" ~ op ~ "rhs.x, y" ~ op ~ "rhs.y, z" ~ op ~ "rhs.z)");
    }
    // Difference (-) between two Point returning a Vec
    pure nothrow Vec opBinary(string op)(in Point rhs) const if (op == "-")
    {
    ...
    pure nothrow Point opBinary(string op)(in float alfa) const if (op == "*")
    {
...
struct Normal
{
    ...
    pure nothrow Normal opBinary(string op)(in float alfa) const if (op == "*")
    {
...
struct Vec2d
{
    ...
    pure nothrow bool uvIsClose(in Vec2d v2d) const
    {
...
pure nothrow Vec[3] createONBFromZ(in Normal n)
{
    Normal normN = n;
